...
repbox_evaluate <- function(input,
                     envir = parent.frame(),
                     enclos = NULL,
                     debug = FALSE,
                     stop_on_error = 0L,
...
evaluate_call <- function(row_ind,
                          call,
                          src = NULL,
                          envir = parent.frame(),
                          enclos = NULL,
  ...
  handle_condition <- function(cond, type) {
    handle_output()
    new = list(cond)
    names(new) = type
    output <<- c(output, new)
  ...
  for (expr in call) {
    ...
    if (ev$visible || multi_args) {
      ...
      value_fun <- if (multi_args) value_handler else {
        function(x, visible, row) value_handler(x)
      }
      handle(pv <- withCallingHandlers(withVisible(
        value_fun(ev$value, ev$visible, row_ind)
      ), warning = wHandler, error = eHandler, message = mHandler))
      handle_output(TRUE)
      # If the return value is visible, save the value to the output
      if (pv$visible) output <- c(output, list(value = pv$value))
    }
  }
